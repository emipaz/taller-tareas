...
Los resolvers son las funciones que se ejecutan cuando se hace una query o mutation.
Conectan las operaciones GraphQL con la lógica de negocio del módulo core.
"""
...
class GraphQLResolvers:
        ...
        try:
            ...
            if filtro:
                ...
                if filtro.activo is not None:
                    usuarios_core = [u for u in usuarios_core if u.activo == filtro.activo]
                if filtro.texto_busqueda:
                    texto = filtro.texto_busqueda.lower()
                    usuarios_core = [u for u in usuarios_core 
                                   if texto in u.nombre.lower() or 
                                   (u.email and texto in u.email.lower())]
        ...
        try:
            ...
            if filtro:
                ...
                if filtro.usuario_id:
                    tareas_core = [t for t in tareas_core if filtro.usuario_id in t.usuarios_asignados]
                ...
                if filtro.fecha_desde:
                    tareas_core = [t for t in tareas_core if t.fecha_creacion >= filtro.fecha_desde]
                
                if filtro.fecha_hasta:
                    tareas_core = [t for t in tareas_core if t.fecha_creacion <= filtro.fecha_hasta]
    ...
    def get_estadisticas_generales(self, info: Info = None) -> EstadisticasGenerales:
        ...
        try:
            ...
            usuarios_activos   = len([u for u in self.gestor.usuarios.values() if u.activo])
    ...
    def get_dashboard(self, info: Info = None) -> DashboardData:
        ...
        try:
            ...
            usuarios_activos = [self._core_usuario_to_graphql(u) 
                              for u in self.gestor.usuarios.values() if u.activo]
    ...
    def crear_usuario(self, input: CrearUsuarioInput, info: Info = None) -> UsuarioMutationResponse:
        ...
        Args:
            input (CrearUsuarioInput): Datos del nuevo usuario:
        ...
        Example:
            input_data = CrearUsuarioInput(
                nombre="Juan Pérez",
                password="password123", 
                rol=RolUsuario.USUARIO,
                email="juan@example.com"
            )
            response = resolver.crear_usuario(input_data, info)
            
        Note:
            Solo usuarios con rol ADMIN pueden crear nuevos usuarios.
            El sistema verifica que el nombre y email sean únicos.
        """
        try:
            current_user = get_current_user(info.context.get("request"))
            ...
            # Crear usuario usando el gestor
            exito, mensaje = self.gestor.crear_usuario(
                nombre   = input.nombre,
            ...
            if exito:
                usuario_core = self.gestor.usuarios[input.nombre]
                return UsuarioMutationResponse(
                    success = True,
                    message = mensaje,
                    usuario = self._core_usuario_to_graphql(usuario_core)
    ...
    def _core_usuario_to_graphql(self, usuario_core: CoreUsuario) -> Usuario:
        ...
        Returns:
            Usuario: Usuario convertido al tipo GraphQL con todos los campos.
            
        Note:
            - Mapea campos del core a estructura GraphQL
            - Agrega campos adicionales con valores por defecto
            - Usa el nombre como ID para mantener compatibilidad
        """
        return Usuario(
            id             = usuario_core.nombre,  # Usar nombre como ID
            nombre         = usuario_core.nombre,
            email          = getattr(usuario_core, 'email', None),
            rol            = RolUsuario(usuario_core.rol),
            fecha_registro = datetime.now(),  # Placeholder
            activo         = getattr(usuario_core, 'activo', True)
        )
    ...
    def _core_tarea_to_graphql(self, tarea_core: CoreTarea) -> Tarea:
        ...
        return Tarea(
            ...
            fecha_finalizacion = getattr(tarea_core, 'fecha_finalizacion', None)
        )
